...
**Primary key convention.** Every primary key and foreign key is `uuid`. The values are
UUIDv7, generated by the application (`Guid.CreateVersion7()`) inside the entity factory
methods — not by the database. There is therefore no identity column and no sequence.

Two consequences worth stating explicitly:

- UUIDv7 carries a millisecond timestamp in its leading bytes, so `ORDER BY id` **in the
  database** is chronological to the millisecond — PostgreSQL compares `uuid` byte-wise. The
  order list and the adjustment history use this as the tiebreaker after `created_at`, and
  both sort server-side. Two caveats: ids created inside the same millisecond have no
  guaranteed order, and .NET's `Guid` comparer does **not** reproduce this ordering (it
  compares the first fields as integers), so sorting ids in memory is not chronological.
- An id exists before `SaveChanges`, so related rows can be linked inside one transaction
  without a round trip to read a generated key.
...
CREATE TABLE plan_adjustments (
    id                          uuid PRIMARY KEY,
    source_production_plan_id  uuid   NOT NULL,
...
CREATE TABLE plan_adjustment_items (
    id                  uuid PRIMARY KEY,
    plan_adjustment_id  uuid   NOT NULL,
